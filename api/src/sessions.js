...
const sessionRestartCounters = new Map() // Track restart attempts per session
...
// Restart backoff: max 5 restarts in 5 minutes, then cooldown 10 minutes
const MAX_RESTARTS = parseInt(process.env.MAX_SESSION_RESTARTS) || 5
const RESTART_WINDOW_MS = parseInt(process.env.RESTART_WINDOW_MS) || 5 * 60 * 1000
const RESTART_COOLDOWN_MS = parseInt(process.env.RESTART_COOLDOWN_MS) || 10 * 60 * 1000
  ...
  if (recoverSessions) {
    waitForNestedObject(client, 'pupPage').then(() => {
      const restartSession = async (sessionId) => {
        // Restart backoff — prevent infinite crash loops
        const now = Date.now()
        if (!sessionRestartCounters.has(sessionId)) {
          sessionRestartCounters.set(sessionId, [])
        }
        const timestamps = sessionRestartCounters.get(sessionId).filter(t => now - t < RESTART_WINDOW_MS)
        timestamps.push(now)
        sessionRestartCounters.set(sessionId, timestamps)

        if (timestamps.length > MAX_RESTARTS) {
          logger.error({ sessionId, restarts: timestamps.length, windowMs: RESTART_WINDOW_MS },
            `Session restart limit reached (${MAX_RESTARTS} in ${RESTART_WINDOW_MS / 1000}s). Cooling down for ${RESTART_COOLDOWN_MS / 1000}s`)
          sessions.delete(sessionId)
          await client.destroy().catch(e => { })
          setTimeout(async () => {
            logger.info({ sessionId }, 'Cooldown expired, attempting restart')
            sessionRestartCounters.delete(sessionId)
            await setupSession(sessionId)
          }, RESTART_COOLDOWN_MS)
          return
        }

        logger.info({ sessionId, attempt: timestamps.length }, 'Restarting session')
        sessions.delete(sessionId)
        await client.destroy().catch(e => { })
        await setupSession(sessionId)
      }
